...
class SimpleCrystalConverter:
    def __init__(
        ...
        self.target_names = target_names if target_names is not None else ['y']
    ...
    def convert(self, d):
        ...
        if self.add_eos_features:
            ...
            edge_attr = torch.tensor(eos.add_eos_features(edge_attr, center_indices, neighbor_indices, distances),
                                     dtype=torch.float)
        ...
        for tname in self.target_names:
            targets.append(getattr(d, tname) if hasattr(d, tname) else 0)
        ...
        for tname, target in zip(self.target_names, targets):
            setattr(data, tname, target)
